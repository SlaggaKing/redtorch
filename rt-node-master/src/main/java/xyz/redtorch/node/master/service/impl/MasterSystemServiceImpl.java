		...
		for (GatewayPo gatewayPo : gatewayPoList) {
			if (gatewayPo.getTargetNodeId().equals(commonReq.getSourceNodeId())) {
				if (gatewayMap.containsKey(gatewayPo.getGatewayId())) {
					...
					if (gatewayPo.getStatus() == ConnectStatusEnum.CONNECTING_VALUE && gateway.getStatus().getNumber() == ConnectStatusEnum.CONNECTED_VALUE) {
					...
					if (gatewayPo.getStatus() == ConnectStatusEnum.CONNECTED_VALUE && gateway.getStatus().getNumber() == ConnectStatusEnum.DISCONNECTED_VALUE) {
			...
			if (gatewayPo.getTargetNodeId() == commonReq.getSourceNodeId()) {
				try {
					...
					gatewaySettingBuilder
							.setGatewayDescription(StringUtils.isAllBlank(gatewayPo.getGatewayDescription()) ? "" : gatewayPo.getGatewayDescription());
