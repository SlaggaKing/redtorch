...
public class MasterTradeCachesServiceImpl implements MasterTradeCachesService, InitializingBean {
		...
		scheduledExecutorService.scheduleAtFixedRate(() -> {
			try {
				...
				for (GatewayPo gateway : gatewayList) {
					if (ConnectStatusEnum.CONNECTED_VALUE == gateway.getStatus() || ConnectStatusEnum.CONNECTING_VALUE == gateway.getStatus()) {
				...
				try {
					// 删除账户缓存
					accountMap = accountMap.entrySet().stream().filter(map -> retainGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
				...
				try {
					// 删除持仓缓存
					positionMap = positionMap.entrySet().stream().filter(map -> retainGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
							.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
				} catch (Exception e) {
				...
				try {
					// 删除定单缓存
					orderMap = orderMap.entrySet().stream().filter(map -> retainGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
							.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
				} catch (Exception e) {
				...
				try {
					// 删除成交缓存
					tradeMap = tradeMap.entrySet().stream().filter(map -> retainGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
							.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
				} catch (Exception e) {
				...
				try {
					// 删除Tick缓存
					tickMap = tickMap.entrySet().stream().filter(map -> retainGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
							.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
				} catch (Exception e) {
				...
				try {
					// 删除Contract缓存
					contractMap = contractMap.entrySet().stream().filter(map -> retainGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
							.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
					// 重建
	...
	public List<OrderField> getOrderList(String operatorId) {
		...
		orderMapLock.lock();
		try {
			orderList = orderMap.values().stream().filter(order -> operatorService.checkReadAccountPermission(operatorId, order.getAccountId()))
	...
	public List<OrderField> getWorkingOrderList(String operatorId) {
		...
		orderMapLock.lock();
		try {
			orderList = workingOrderMap.values().stream().filter(order -> operatorService.checkReadAccountPermission(operatorId, order.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<OrderField> queryOrderListByUnifiedSymbol(String operatorId, String unifiedSymbol) {
		...
		orderMapLock.lock();
		try {
			orderList = orderMap.values().stream().filter(order -> order.getContract().getUnifiedSymbol().equals(unifiedSymbol)
					&& operatorService.checkReadAccountPermission(operatorId, order.getAccountId())).collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TradeField> getTradeList(String operatorId) {
		...
		tradeMapLock.lock();
		try {
			tradeList = tradeMap.values().stream().filter(trade -> operatorService.checkReadAccountPermission(operatorId, trade.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TradeField> queryTradeListByUnifiedSymbol(String operatorId, String unifiedSymbol) {
		...
		tradeMapLock.lock();
		try {
			tradeList = tradeMap.values().stream().filter(trade -> trade.getContract().getUnifiedSymbol().equals(unifiedSymbol)
					&& operatorService.checkReadAccountPermission(operatorId, trade.getAccountId())).collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TradeField> queryTradeListByOrderId(String operatorId, String orderId) {
		...
		try {
			tradeList = tradeMap.values().stream()
					.filter(trade -> trade.getOrderId().equals(orderId) && operatorService.checkReadAccountPermission(operatorId, trade.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TradeField> queryTradeListByOriginOrderId(String operatorId, String originOrderId) {
		...
		tradeMapLock.lock();
		try {
			tradeList = tradeMap.values().stream().filter(
					trade -> trade.getOriginOrderId().equals(originOrderId) && operatorService.checkReadAccountPermission(operatorId, trade.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<PositionField> getPositionList(String operatorId) {
		...
		positionMapLock.lock();
		try {
			positionList = positionMap.values().stream().filter(position -> operatorService.checkReadAccountPermission(operatorId, position.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<PositionField> queryPositionListByAccountId(String operatorId, String accountId) {
		...
		try {
			if (operatorService.checkReadAccountPermission(operatorId, accountId)) {
				positionList = positionMap.values().stream().filter(positionField -> positionField.getAccountId().equals(accountId))
						.collect(Collectors.toList());
			}
	...
	public List<PositionField> queryPositionListByUnifiedSymbol(String operatorId, String unifiedSymbol) {
		...
		positionMapLock.lock();
		try {
			positionList = positionMap.values().stream().filter(position -> position.getContract().getUnifiedSymbol().equals(unifiedSymbol)
					&& operatorService.checkReadAccountPermission(operatorId, position.getAccountId())).collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<AccountField> getAccountList(String operatorId) {
		...
		accountMapLock.lock();
		try {
			accountList = accountMap.values().stream().filter(account -> operatorService.checkReadAccountPermission(operatorId, account.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<AccountField> queryAccountListByAccountCode(String operatorId, String accountCode) {
		...
		try {
			accountList = accountMap.values().stream()
					.filter(account -> account.getCode().equals(accountCode) && operatorService.checkReadAccountPermission(operatorId, account.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TickField> getTickList(String operatorId) {
		...
		tickMapLock.lock();
		try {
			tickList = tickMap.values().stream().filter(tick -> operatorService.checkSubscribePermission(operatorId, tick.getContract().getUnifiedSymbol()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<ContractField> queryContractListByUnifiedSymbol(String operatorId, String unifiedSymbol) {
		...
		contractMapLock.lock();
		try {
			contractList = contractMap.values().stream().filter(contractField -> contractField.getUnifiedSymbol().equals(unifiedSymbol))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<ContractField> queryContractListByGatewayId(String operatorId, String gatewayId) {
		...
		contractMapLock.lock();
		try {
			contractList = contractMap.values().stream().filter(contractField -> contractField.getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public void cacheOrder(OrderField order) {
		...
		try {
			...
			if (!orderMap.containsKey(orderId) || (!orderMap.get(orderId).getOrderStatus().equals(OrderStatusEnum.ALL_TRADED)
	...
	public void clearAndCacheOrderList(List<OrderField> orderList, int sourceNodeId) {
		...
		try {
			...
			orderMap = orderMap.entrySet().stream().filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
			for (OrderField order : orderList) {
				orderMap.put(order.getOrderId(), order);
			}
	...
	public void clearAndCacheTradeList(List<TradeField> tradeList, int sourceNodeId) {
		...
		try {
			...
			tradeMap = tradeMap.entrySet().stream().filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
			for (TradeField trade : tradeList) {
				tradeMap.put(trade.getTradeId(), trade);
			}
	...
	public void clearAndCacheContractList(List<ContractField> contractList, int sourceNodeId) {
		...
		try {
			...
			contractMap = contractMap.entrySet().stream().filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
			mixContractMap = mixContractMap.entrySet().stream().filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
			for (ContractField contract : contractList) {
				contractMap.put(contract.getContractId(), contract);
				mixContractMap.put(contract.getUnifiedSymbol(), contract);
	...
	public void clearAndCachePositionList(List<PositionField> positionList, int sourceNodeId) {
		...
		try {
			...
			positionMap = positionMap.entrySet().stream().filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
			for (PositionField position : positionList) {
				positionMap.put(position.getPositionId(), position);
			}
	...
	public void clearAndCacheAccountList(List<AccountField> accountList, int sourceNodeId) {
		...
		try {
			...
			accountMap = accountMap.entrySet().stream().filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	...
	public void clearAndCacheTickList(List<TickField> tickList, int sourceNodeId) {
		...
		try {
			...
			tickMap = tickMap.entrySet().stream().filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
			for (TickField tick : tickList) {
				tickMap.put(tick.getDataSourceId(), tick);
			}
	...
	public void clearAllCachesByGatewayId(String gatewayId) {
		...
		try {
			// 删除账户缓存
			accountMap = accountMap.entrySet().stream().filter(map -> !map.getValue().getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
		} catch (Exception e) {
			logger.error("删除账户缓存异常", e);
		} finally {
		...
		try {
			// 删除持仓缓存
			positionMap = positionMap.entrySet().stream().filter(map -> !map.getValue().getContract().getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
		} catch (Exception e) {
			logger.error("删除持仓缓存异常", e);
		} finally {
		...
		try {
			// 删除定单缓存
			orderMap = orderMap.entrySet().stream().filter(map -> !map.getValue().getContract().getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
		} catch (Exception e) {
			logger.error("删除定单缓存异常", e);
		} finally {
		...
		try {
			// 删除成交缓存
			tradeMap = tradeMap.entrySet().stream().filter(map -> !map.getValue().getContract().getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
		} catch (Exception e) {
			logger.error("删除成交缓存异常", e);
		} finally {
		...
		try {
			// 删除Tick缓存
			tickMap = tickMap.entrySet().stream().filter(map -> !map.getValue().getContract().getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
		} catch (Exception e) {
			logger.error("删除Tick缓存异常", e);
		} finally {
		...
		try {
			// 删除Contract缓存
			contractMap = contractMap.entrySet().stream().filter(map -> !map.getValue().getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
