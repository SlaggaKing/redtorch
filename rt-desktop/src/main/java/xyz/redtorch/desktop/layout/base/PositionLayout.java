...
public class PositionLayout {
	...
	private Logger logger = LoggerFactory.getLogger(PositionLayout.class);
	...
	public void fillingData() {
		...
		if (!showMergedPosition) {
			...
			for (PositionField position : newPositionList) {
				...
				String key = position.getContract().getUnifiedSymbol() + "#" + position.getDirectionValue();
				...
				if (mergedPositionFieldBuilderMap.containsKey(key)) {
					...
					if (positionInt != 0) {
						...
						double price = (positionFieldBuilder.getPrice() * positionFieldBuilder.getPosition() + position.getPrice() * position.getPosition())
								/ positionInt;
					...
					positionFieldBuilder.setDirection(position.getDirection());
	...
	private void createLayout() {
		...
		unifiedSymbolCol.setCellValueFactory(feature -> {
			...
			try {
				...
				Text shortNameText = new Text(position.getContract().getShortName());
				vBox.getChildren().add(unifiedSymbolText);
				vBox.getChildren().add(shortNameText);

				if (guiMainService.getSelectedContract() != null
						&& guiMainService.getSelectedContract().getUnifiedSymbol().equals(position.getContract().getUnifiedSymbol())) {
		...
		directionCol.setComparator((Text t1, Text t2) -> {
			try {
				...
				return Integer.compare(position1.getDirection().getNumber(), position2.getDirection().getNumber());
			} catch (Exception e) {
				logger.error("排序错误", e);
			}
			return 0;
