	...
	public void fillingData() {
		...
		for (TradeField trade : this.tradeList) {
			if (guiMainService.getSelectedAccountIdSet().isEmpty() || guiMainService.getSelectedAccountIdSet().contains(trade.getAccountId())) {
				if (showRadioValue == SHOW_ALL) {
					...
					if (DirectionEnum.LONG_VALUE == trade.getDirectionValue()) {
						newTradeList.add(trade);
					}
				} else if (showRadioValue == SHOW_SHORT) {
					if (DirectionEnum.SHORT_VALUE == trade.getDirectionValue()) {
						newTradeList.add(trade);
					}
				}
			}
		...
		if (!showMergedChecked) {
			...
			for (TradeField trade : newTradeList) {
				String key = trade.getContract().getUnifiedSymbol() + "#" + trade.getDirectionValue() + "#" + trade.getOffsetValue();
				...
				if (mergedTradeFieldBuilderMap.containsKey(key)) {
					...
					tradeFieldBuilder.setOffset(trade.getOffset());
	...
	private void createLayout() {
		...
		unifiedSymbolCol.setCellValueFactory(feature -> {
			...
			try {
				...
				Text shortNameText = new Text(trade.getContract().getShortName());
				vBox.getChildren().add(unifiedSymbolText);
				vBox.getChildren().add(shortNameText);

				if (guiMainService.getSelectedContract() != null
						&& guiMainService.getSelectedContract().getUnifiedSymbol().equals(trade.getContract().getUnifiedSymbol())) {
		...
		offsetCol.setCellValueFactory(feature -> {
			...
			try {
				...
				if (trade.getOffset() == OffsetEnum.CLOSE) {
					offset = "平";
				} else if (trade.getOffset() == OffsetEnum.CLOSE_TODAY) {
					offset = "平今";
				} else if (trade.getOffset() == OffsetEnum.CLOSE_YESTERDAY) {

					offset = "平昨";
					;
				} else if (trade.getOffset() == OffsetEnum.OPEN) {
		...
		tradeTimeCol.setPrefWidth(60);
		tradeTimeCol.setCellValueFactory(feature -> {
			String tradeTime = "";
			try {
				tradeTime = feature.getValue().getTradeTime();
			} catch (Exception e) {
				logger.error("渲染错误", e);
			}
		...
		TableColumn<TradeField, String> adapterOrderIdCol = new TableColumn<>("定单编号");
		...
		TableColumn<TradeField, String> adapterTradeIdCol = new TableColumn<>("成交编号");
		adapterTradeIdCol.setPrefWidth(60);
