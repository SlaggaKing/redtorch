...
public class TickLayout {
	...
	public void fillingData() {
		...
		for (TickField tick : tickObservableList) {
			if (selectedTickUnifiedSymbolSet.contains(tick.getContract().getUnifiedSymbol())) {
				tickTableView.getSelectionModel().select(tick);
				newSelectedTickIdSet.add(tick.getContract().getUnifiedSymbol());
			}
		}
	...
	private void createLayout() {
		...
		unifiedSymbolCol.setCellValueFactory(feature -> {
			...
			try {
				...
				Text unifiedSymbolText = new Text(tick.getContract().getUnifiedSymbol());
				Text shortNameText = new Text(tick.getContract().getShortName());
				vBox.getChildren().add(unifiedSymbolText);
				vBox.getChildren().add(shortNameText);

				if (guiMainService.getSelectedContract() != null
						&& guiMainService.getSelectedContract().getUnifiedSymbol().equals(tick.getContract().getUnifiedSymbol())) {
		...
		unifiedSymbolCol.setComparator((Pane p1, Pane p2) -> {
			try {
				...
				return StringUtils.compare(tick1.getContract().getUnifiedSymbol(), tick2.getContract().getUnifiedSymbol());
			} catch (Exception e) {
				logger.error("排序错误", e);
			}
			return 0;
		...
		lastPriceCol.setCellValueFactory(feature -> {
			...
			try {
				...
				int dcimalDigits = CommonUtils.getNumberDecimalDigits(tick.getContract().getPriceTick());
				if (dcimalDigits < 0) {
					dcimalDigits = 0;
				}
		...
		tickTableView.setOnMousePressed(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				...
				for (TickField row : selectedItems) {
					selectedTickUnifiedSymbolSet.add(row.getContract().getUnifiedSymbol());
				}
			}
		});
			...
			row.setOnMousePressed(event -> {
				if (!row.isEmpty() && event.getButton() == MouseButton.PRIMARY && event.getClickCount() == 1) {
					...
					for (TickField tick : selectedItems) {
						selectedTickUnifiedSymbolSet.add(tick.getContract().getUnifiedSymbol());
					}
