...
public class ContractLayout {
	...
	public void fillingData() {
		...
		if ((this.filterCurrency.equals("全部")) && (this.filterExchange.equals("全部")) && (this.filterProductType.equals("全部"))
				&& (this.filterFuzzySymbol.equals("")) && (this.filterLastTradeDateOrContractMonth.isEmpty()) && (this.filterThirdPartyId.isEmpty())
			...
			for (ContractField contract : this.contractList) {
				...
				flag = flag && (this.filterProductType.equals("全部") || contract.getProductType().getValueDescriptor().getName().equals(filterProductType));

				flag = flag && (this.filterFuzzySymbol.isEmpty() || contract.getSymbol().toLowerCase().contains(this.filterFuzzySymbol.toLowerCase()));

				flag = flag && (this.filterLastTradeDateOrContractMonth.isEmpty()
						|| contract.getLastTradeDateOrContractMonth().contains(this.filterLastTradeDateOrContractMonth));

				flag = flag && (this.filterUnderlyingSymbol.isEmpty()
						|| contract.getUnderlyingSymbol().toLowerCase().contains(this.filterUnderlyingSymbol.toLowerCase()));
				...
				flag = flag && (this.filterName.isEmpty() || contract.getFullName().toLowerCase().contains(this.filterName.toLowerCase())
						|| contract.getShortName().toLowerCase().contains(this.filterName.toLowerCase()));
	...
	private void createLayout() {
		...
		shortNameCol.setCellValueFactory(feature -> {
			String shortName = "";
			try {
				shortName = feature.getValue().getShortName();
			} catch (Exception e) {
				logger.error("渲染错误", e);
			}
		...
		productTypeCol.setCellValueFactory(feature -> {
			String productType = "";
			try {
				productType = feature.getValue().getProductType().getValueDescriptor().getName();
			} catch (Exception e) {
				logger.error("渲染错误", e);
			}
		...
		optionTypeCol.setCellValueFactory(feature -> {
			String optionType = "";
			try {
				optionType = feature.getValue().getOptionType().getValueDescriptor().getName();
			} catch (Exception e) {
				logger.error("渲染错误", e);
			}
		...
		for (int i = 0; i < ProductTypeEnum.values().length - 1; i++) {
			ProductTypeEnum productType = ProductTypeEnum.values()[i];
