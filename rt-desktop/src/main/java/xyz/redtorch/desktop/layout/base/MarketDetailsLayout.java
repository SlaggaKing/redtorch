...
public class MarketDetailsLayout {
	...
	private Text volumeChangeText = new Text("-");
	...
	private Text dayOpenInterestChangeText = new Text("-");
	...
	public void fillingData() {
		...
		if (this.tick == null) {
			...
			this.dayOpenInterestChangeText.setText("-");
			...
			ContractField contract = tick.getContract();
			...
			contractSymbolText.setText(contract.getSymbol());
			contractNameText.setText(contract.getShortName());
			...
			int volumeChange = tick.getVolumeChange();
			this.volumeChangeText.setText(String.format("%,d", volumeChange));
			...
			if (tick.getOpenInterest() == Double.MAX_VALUE || tick.getPreOpenInterest() == Double.MAX_VALUE) {
				...
				this.dayOpenInterestChangeText.setText(String.format("%,.0f", tick.getOpenInterest() - tick.getPreOpenInterest()));
			}
	...
	private void createLayout() {
		...
		vBox.setStyle("-fx-border-color: rgb(200, 200, 200);-fx-border-style: solid;-fx-border-width: 0 1 0 0;");
		...
		Label volumeChangeLabel = new Label("现手");
		volumeChangeLabel.getStyleClass().add("trade-label");
		volumeChangeLabel.setPrefWidth(labelWidth);
		volumeChangeText.setWrappingWidth(valueWidth);
		volumeChangeText.setTextAlignment(TextAlignment.RIGHT);
		volumeChangeText.getStyleClass().add("trade-remind-color");
		...
		line2Hbox.getChildren().addAll(volumeChangeLabel, volumeChangeText, priceDiffLabel, priceDiffText);
		HBox.setMargin(volumeChangeText, rightInsets);
		HBox.setMargin(volumeChangeLabel, leftInsets);
		...
		Label dayOpenInterestChangeLabel = new Label("日增");
		dayOpenInterestChangeLabel.getStyleClass().add("trade-label");
		dayOpenInterestChangeLabel.setPrefWidth(labelWidth);
		dayOpenInterestChangeText.setWrappingWidth(valueWidth);
		dayOpenInterestChangeText.setTextAlignment(TextAlignment.RIGHT);
		dayOpenInterestChangeText.getStyleClass().add("trade-remind-color");
		...
		line5Hbox.getChildren().addAll(dayOpenInterestChangeLabel, dayOpenInterestChangeText, lowPriceLabel, lowPriceText);
		HBox.setMargin(dayOpenInterestChangeText, rightInsets);
		HBox.setMargin(dayOpenInterestChangeLabel, leftInsets);
